...
async function setPythonEnvironment(): Promise<void> {
    ...
    const excludeDirs = code.workspace.getConfiguration('python-environment').get<string[]>('excludedFolders') || EXCLUDE_DIRS; //FIXME: How to set this settins into the user settings vscode interface?
...
export function activate(context: code.ExtensionContext){
    let command = code.commands.registerCommand('python-environment.setPythonEnvironment',setPythonEnvironment);
